...
F = butdiff.VertexConstructor(mask, lin_scale, H);
P = SymMatrix(F, prob_jump);
...
prob_ret = zeros(max_time, 1);

prob_vec = zeros(size(P, 1), 1);

start_idx = floor(numel(prob_vec)/2);
prob_vec(start_idx) = 1;

prob_vec = vpa(prob_vec, digs);
for idx = 1:max_time
    prob_vec = P * prob_vec;
...
figure
plot(diff(double(lambda_all)), ". blue")
